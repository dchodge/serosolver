...
#library(serosolver)
#devtools::install("/Users/davidhodgson/Dropbox/My Mac (David’s MacBook Air)/Documents/research/flu/serosolver_vac")
#require(serosolver)
...
chain_path_real_pt <- paste0(chain_path, "cs2_real_pt/")

## Generate results in parallel, does work atm not quite why
foreach(x = filenames, .packages = c('serosolver', 'data.table', 'plyr')) %dopar% {
#for (x in filenames)
  ## Not all random starting conditions return finite likelihood, so for each chain generate random
  ## conditions until we get one with a finite likelihood
  start_prob <- -Inf
  ...
  res <- run_MCMC_pt(
                  ...
                  antigenic_map = antigenic_map,
                  start_inf_hist = start_inf, 
                  # Must be defined as a list so that temperature can be passed!
                  mcmc_pars = list("iterations"=50000,"adaptive_period"=10000,"thin"=100,
                                "thin_hist"=100,"save_block"=10,
                                "inf_propn"=1, "hist_sample_prob"=1,
                                "hist_switch_prob"=0.8, "year_swap_propn"=1,
                                "temperature" = seq(1, 10, 1),
...
## Checks on fitting procedures on vingette 2
...
post_normal <- load_mcmc_chains(location = here::here("inst", "extdata", "cs2_real"), thin = 1, burnin = 0,
                               par_tab = par_tab, unfixed = FALSE, convert_mcmc = TRUE)
...
post_normal_pt <- load_mcmc_chains(location = here::here("inst", "extdata", "cs2_real_pt"), thin = 1, burnin = 0,
                               par_tab = par_tab, unfixed = FALSE, convert_mcmc = TRUE)
