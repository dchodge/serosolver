...
example_par_tab <- example_par_tab[example_par_tab$names != "phi",]
y <- get_titre_predictions(example_theta_chain, example_inf_chain, example_titre_dat,
                           ...
                           par_tab = example_par_tab, expand_titredat = FALSE)
...
model_func <- create_posterior_func(par_tab = par_tab,
                            ...
                            custom_ab_kin_func = titre_data_fast_individual_base,
                            custom_antigenic_maps_func = make_antigenic_maps_vac1) # function in posteriors.R
#
```
...
res <- foreach(x = filenames, .packages = c('serosolver','data.table','plyr')) %dopar% {
  ...
  res <- run_MCMC(par_tab = start_tab,
                  ...
                  mcmc_pars = list("iterations"=2000,"popt"=0.44,"popt_hist"=0.44,
                                "opt_freq"=10,"thin"=1,"adaptive_period"=500, 
                                "save_block"=10, "thin_hist"=10,"hist_sample_prob"=1,
...
res <- foreach(x = filenames, .packages = c('serosolver','data.table','plyr')) %dopar% {
  ...
  res <- run_MCMC_pt(par_tab = start_tab,
                  ...
                  strain_isolation_times = strain_isolation_times,
                  start_inf_hist = start_inf,
                  mcmc_pars = list("iterations"=10000,"popt"=0.44,"popt_hist"=0.44,
